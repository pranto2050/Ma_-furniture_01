...
const FilterSection: React.FC<FilterSectionProps> = ({ onApply }) => {
  ...
  return (
    <section className="bg-white py-10 -mt-12.5 relative z-20 max-w-container mx-auto px-5">
      <div className="bg-white p-7.5 rounded-[20px] shadow-[0_15px_50px_rgba(0,0,0,0.1)] border border-[#f0f0f0]">
        <form className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1fr_auto] gap-5 items-center" onSubmit={handleFilterChange}>
          <div className="flex flex-col gap-2.5 relative">
            ...
            <div className="relative flex items-center">
                <i className="fas fa-search absolute left-3.75 text-gray-medium"></i>
              <input 
                type="text" 
                placeholder="পণ্যের নাম বা আইডি দিয়ে খুঁজুন..." 
                className="w-full pl-10 pr-3.75 py-3.5 border-2 border-[#f5f5f5] rounded-xl outline-none text-[0.95rem] bg-[#fcfcfc] transition-all focus:border-accent focus:bg-white focus:shadow-[0_0_0_4px_rgba(212,168,83,0.1)]"
                value={searchQuery}
          ...
          <div className="flex flex-col gap-2.5 relative">
            ...
            <input 
              type="number" 
              placeholder="৳ ০" 
              className="w-full px-3.75 py-3.5 border-2 border-[#f5f5f5] rounded-xl outline-none text-[0.95rem] bg-[#fcfcfc] transition-all focus:border-accent focus:bg-white"
              value={minPrice}
          ...
          <button 
            type="submit"
            className="bg-primary text-white w-13.75 h-13.75 rounded-[15px] flex items-center justify-center cursor-pointer transition-all hover:bg-primary-dark hover:-translate-y-0.75 hover:rotate-90 hover:shadow-[0_15px_25px_rgba(139,69,19,0.3)] mt-6"
          >
