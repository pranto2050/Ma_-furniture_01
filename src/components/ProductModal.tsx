...
const ProductModal: React.FC<ProductModalProps> = ({ product, onClose, onNavigate, hidePrice }) => {
  ...
  return (
    <div className={`fixed inset-0 bg-transparent z-2000 flex items-center justify-center transition-opacity duration-300 ${product ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
      <div
        className={`bg-white w-[95%] max-w-275 max-h-[95vh] rounded-[25px] relative flex flex-col md:flex-row overflow-hidden transition-all duration-500 shadow-2xl ${product ? 'scale-100' : 'scale-90'}`}
      >
